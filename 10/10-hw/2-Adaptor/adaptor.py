...
class Adapter:
    ...
    def upload_documents(self, xml_files_to_upload):
        transformed_files_to_json = []
        for file in xml_files_to_upload:
            transformed_files_to_json.append(file.replace('xml', 'json'))
        return self.dh.upload_documents(transformed_files_to_json)

    def get_documents(self, document_ids):
        return self.dh.get_documents(document_ids)
